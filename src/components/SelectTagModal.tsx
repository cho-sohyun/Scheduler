  ...
  return (
    <div className="z-10 fixed top-0 left-0 w-full h-full flex items-center justify-center bg-black bg-opacity-50">
      <div className="bg-white w-[420px] h-[180px] p-4 rounded-xl modal-box">
        ...
        <h2 className="text-sm font-bold mb-6">태그 선택</h2>
        <div className="flex justify-start space-x-4">
          {tagOptions.map((tag) => (
            <div
              key={tag.name}
              onClick={() => onSelect(tag.name, tag.color)}
              className="w-[80px] h-[80px] flex flex-col justify-center items-center p-2 rounded-md bg-gray-100 cursor-pointer"
            >
              <button
                className={`w-[20px] h-[20px] p-2 rounded-full ${tag.color}`}
              />
              <p className="text-center text-sm text-[#13133d] text-nowrap mt-1">
                {tag.name}
              </p>
            </div>
          ))}
